...
public class Facebook {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Thread.sleep(1000);
		act.moveToElement(days).click().sendKeys(Keys.HOME).perform();
		Thread.sleep(1000);
		//days.sendKeys("15");
		for(int i=1;i<=14;i++) {
		...
		act.moveToElement(month).click().sendKeys(Keys.HOME).perform();
		Thread.sleep(1000);
		month.sendKeys("April");

		//Performing a year list section
		WebElement year=driver.findElement(By.xpath("//select[@id='year']"));
		act.moveToElement(year).click().sendKeys(Keys.HOME).perform();
		Thread.sleep(1000);
		year.sendKeys("1991");
